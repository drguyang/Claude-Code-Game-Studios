...
三轮复核(2026-09-14)末段明写「21a 仍 **In Review,不得标 Approved**」,并建议「第四轮复核应以『逐条核对冻结裁决表 ↔ 可执行体』为唯一靶子」。**用户裁定:接受三轮修订落盘状态、以显式风险接受标记 Approved,不开第四轮**(承 37 / 42 / 51 的「免二轮 = 显式风险接受」先例)。

**⚠️ 这条结案是显式风险接受,不是「已核对」。** 三轮指出的**传导缺口**(一处声明、另一处漏改 —— §D-21-x / §Tuning Knobs / §AC / `entities.yaml` 四方漂移)在 Approval 时**未独立复核清点**,转由**实现期第一道构建期门**兜底。

**实现期门(不阻塞 Approval,阻塞开工)**:`D-21-21`(守恒律两侧封)· `D-21-24`(F5 可感知地板)· `D-21-25`(9 侧字段名对齐 `drug_potency`/`half_life`)· `D-21-26`/`D-21-27`(`instance_id` 权威契约)· `D-21-28`(Craft 事件总序键)+ `AC-21a-53`(改型后)/ `AC-21a-56…62`。

**重开触发条件(四者任一)**:① 实现期构建期门(守恒律上界 / `EFF_MIN` / `ordinal` 类)拦下运行时击穿 = 冻结裁决表与可执行体不一致;② `instance_id` / `structure_id` 高水位在迁移后出现复用;③ F5 偏移落入 9 的噪声带(可感知地板失效);④ 9 侧字段名与 `drug_potency` / `half_life` 对不上。
