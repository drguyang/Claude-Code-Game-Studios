...
//          · Story 002 schema 落型(AC-21a-60「P0 只许 half_life」的**校验**归 Story 006,本故事不实现)。
//
// ⚠️ 四轴枚举**定义**在此(闭集词汇),但 P0 收窄是**数据期断言**不是类型期断言 ——
//    类型允许四个成员编译通过;构建期拒 onset/peak/elimination 的逻辑属 AC-21a-60(出范围)。
...
namespace DaYiJingCheng.Sim.Contracts
{
    ...
    public enum QualityAxis
    {
        ...
        HalfLife = 0,
        /// <summary>起效(JSON: <c>"onset"</c>)—— P1a(P0 拒 = AC-21a-60,归 Story 006)。</summary>
        Onset = 1,
