...
> **Last Updated**: 2026-09-15(ADR-021 POI 状态所有权落盘 —— 结清洞 H2,TR +9)
...
> **基线**: 196 条 TR(首建 148 条;ADR-017/018/019 轮增补 6 条;ADR-020 轮增补 9 条;
> **#13 GDD 回溯追加 24 条** —— 按 ADR-016 Ordering Note 的约定,与 21a / 52 同法;
> **ADR-021 代生追加 9 条** —— 6 无独立 GDD,同 1 / 2 例)
...
| `patient-ai.md` | 13 病人 AI 与行为 | 24 | **18** | **1** | **5** |
| *(无独立 GDD)* | **6 世界与生态区** | **9** | **8** | 0 | **1** |
| **合计** | | **196** | **109** | **15** | **72** |
...
| TR-patient-006 | 行为模拟范围 = **在场病人**;13 不自建在场定义 | — | ⚠️ **partial —— 根因 = 9 的 `OQ-8`**(在场判定无值源);13 只裁自己那一半 |
...
| TR-patient-013 | 出现率上限归 9(`TR-disease-021`);13 不自主生成 / 删除病人 | ADR-016 | ✅ 链路 = 52 → 37 → 13(§八);AC-13-C4;**硬依赖** `TR-disease-021`(仍 gap) |
...
| TR-patient-018 | 咳嗽 / 呻吟 cue(13 → 44):`AudioCueDto`,不携带「状态变化」语义 | ADR-018 | ✅ §一 / §四;AC-44-09 白名单;**`Tier` 13 不填**(= 44 的听诊精度档,与距离无关) |
...
## 10. 世界与生态区 `design/gdd/systems-index.md`(#6)| 9 条

> **2026-09-15 ADR-021 落盘时生成** —— 结清三方复核登记的洞 **H2(POI 状态无拥有者)**。
> 需求文本**代生**(同 §8 的 1 / 2 例):**6 无独立 GDD**(在 P0 队列),由 ADR-021 裁决 +
> `systems-index.md` §11 H2 条目生成,**6 撰写 GDD 时须回溯核对**。`system:` slug = `world-eco`。
...
| TR-worldeco-007 | 转移数**有界**:≤ `\|POI\| × \|STATE\|` | ADR-021 + ADR-009 | ✅ §决定④;补 ADR-009 §六 有界性 |
| TR-worldeco-008 | 52 的 `spawn_anchor` **读定义不读状态** | ADR-021 | ✅ §决定⑤;否则抽池 = 状态的函数 |
| TR-worldeco-009 | `PoiState` **枚举值** + **状态机规则**(转移合法性 / 可逆性) | — | ❌ **ADR-021 刻意不定**(避越权,同 ADR-009 §三 教训);归 6 的 GDD |
