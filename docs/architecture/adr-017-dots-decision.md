...
`plugins/dots-entities.md` 所载的 DOTS 甜点区(**阈值待用户裁定**)。
...
| **Knowledge Risk** | **LOW —— 本裁决不依赖任何 post-cutoff API。** 判据是**边界**(门 A)与**规模未定**(OQ-8)两件结构性事实,与 Unity 6.3 的 DOTS 具体行为无关。**复评门一旦触发**,才需实测 post-cutoff DOTS(Entities 1.3+ / 6.3 的「production-ready DOTS」),届时按本 ADR 的复评协议另开 ADR |
...
| **Verification Required** | ① **门 A ↔ `Unity.Entities` 冲突的正式记载**(本 ADR §二 已记,实现期须有一条 EditMode 断言把「sim 程序集程序集引用集恰好 = {BCL}」固定下来,把冲突从「约定」升为「构建失败」);② **OQ-8 标定**(病人数范围 —— R-11 复评门与 ADR-005 性能表的**共同前置输入**);③ 复评门触发后的 post-cutoff DOTS 实测(仅在触发时) |
...
| **Enables** | **ADR-004 结案**(`technical-preferences.md` 的待建项就此兑现)· **9 / 13 / 27 / 34 / 36 的性能预算可以「纯 C# 值类型」为唯一假设**(不再需要对冲 DOTS 迁移)· **ADR-005 性能表的量纲确定**(实体数 × tick 频率 × 单次求值 —— 系数待 OQ-8)|
| **Blocks** | **不阻塞任何实现** —— 本裁决是「维持现状 + 关门」。34 公卫 / 36 NPC 的实现**从纯 C# 出发**,无需等待 DOTS 结论 |
| **Ordering Note** | 本 ADR 是 **R-11 的落点**,**兑现 ADR-004**。**OQ-8 是它的复评前置**,但**不是它的实现前置** —— 换言之:即便 OQ-8 今天标定出「病人数 = 5000」,sim 侧**仍然**不上 DOTS(门 A)。OQ-8 只可能推动**表现层**复评 |
...
| 量 | 现状 | 出处 |
|----|------|------|
| 病人数 | **从未定义**;「在场才模拟 vs 全域模拟」未定 ⇒ 登记为 **OQ-8**;GDD 自陈「规则八要生态区级普查,概念又说疫情表现为散落的病例,**差 2–3 个数量级**」;并**正式撤销**了 ADR-005 性能表里的「数十病人」自引 | `disease-simulation.md:560-564` · `:1329-1335`(OQ-8 全文) |
...
(触发 = OQ-8 标定后同场实体数进入 DOTS 甜点区,阈值待用户裁定);触发前表现层同样不引入 DOTS。
...
| 项 | 内容 |
|----|------|
| **触发输入** | **OQ-8 标定完成**(`disease-simulation.md:1329-1335`):「在场才模拟 vs 全域模拟」定案 ⇒ 同场实体数有数 |
| **触发判据** | 同场实体数 ≥ **[阈值 — 待用户裁定]** **或** 表现层帧时间占比 ≥ **[阈值 — 待用户裁定]** |
...
> **阈值留白是有意的。** 本项目的一贯纪律:**数值用户自己调**。此处同理 ——
> 「多少实体算多」的量级判断属用户,本 ADR 只固定**机制**(谁触发、判定什么、走什么流程),
> 不替用户拍定阈值。`plugins/dots-entities.md` 的「1000s–10,000s」是**引擎参考的量级提示**,
> 不是本项目的裁定值。
...
# DOTS_REVIEW_GATE:
#   input:   OQ-8 标定完成(disease-simulation.md)
#   trigger: concurrent_entity_count >= [待用户裁定]  OR  presentation_frame_share >= [待用户裁定]
...
4. **表现层的性能手段**(DOTS 之外):GPU Resident Drawer(Unity 6.3 已有,URP)·
   ...
   **NavMesh 只在表现层**)。
5. 复评门触发时,**先标定 OQ-8**(它是前置输入,不是可选项),再判阈值,再另开 ADR。
...
| 阈值留白被误读为「不需要阈值」 | 低 | 中 | §三 明写「待用户裁定」并注明「数值用户自己调」的纪律 |
| OQ-8 长期不标定 ⇒ 复评门与 ADR-005 性能表**同时**悬空 | 中 | 中 | 已在 `adr-005` 性能表加注(OQ-8 是该表的量纲输入);两处指向同一前置,便于一并解决 |
...
- [ ] **复评门登记存在**:`technical-preferences.md` 含 `DOTS_REVIEW_GATE` 条目,
      字段 = 输入(OQ-8)/ 判据(阈值占位)/ 作用面(表现层)/ 执行方式(另开 ADR)
- [ ] **阈值留白被识别为待用户裁定**:登记值处明写「待用户裁定」,非默认为 0 或空
