...
> **残留执行义务**(不在本注记内落地,登记为待办;状态为 2026-09-21 第二十七批回填):
> ① ✅ **已兑现(第二十七批)** —— `entities.yaml` 已增列 `next_player_id` 计数器条目
> (非 Kind,不占 V-2 条目数;共用 id 空间 + None=−1 不占用 + 高水位单点守卫均已随条目登记);
> ② **仍 open** —— 45 网络层的「加入会话 → 铸造 → 上行」契约件(归 45 GDD 轮 + ADR-001 窄修订同批);
> ③ ✅ **已兑现(第二十七批)** —— 7a `persistence-service.md` 规则九末段已显式声明
> 「不独立快照玩家 id 计数器」(少一份可失步的状态,承 Alternative 4 理据;高水位可由事件流重构)。
