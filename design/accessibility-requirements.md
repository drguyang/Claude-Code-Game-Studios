...
> **Last Updated**: 2026-09-20(**2026-09-21 第二十八批「42 修订轮」回刷**:Visual Accessibility 表
> :83「世界内纸面」行**并归** :82「纸面 UI」档(OQ-C3 结案,`ADR-013 §十-B`)—— 世界内的纸在
> P0 的读取形态 = 42 UI Toolkit 近景模态,故两档合一。**档位承诺(Standard + L-1/L-2)零改动**,
> 矩阵行数零增删(83 转为指向 82 的归档行),无任何一项翻绿 —— 全部仍 `Not Started` / 阻塞已解但未测)
...
|---------|-------------|-------|--------|---------------------|
| 最小字号 —— 菜单 / 纸面 UI(⭑ **含世界内纸的 42 近景模态**,原 :83 单列行已并归本行) | Standard | 全部 UI Toolkit 屏(脉案 / 存档位 / 主菜单 / **教学纸近景 `PaperCloseup48`**) | Not Started | ≥24px @1080p 基准;**纸纹底上的墨色字**对比另见下行。落点 = ADR-013 §三 自建 USS 元件库的主题变量(字号走 `--font-size-*` 变量,不散写) |
| ~~最小字号 —— 世界内纸面(48 教学 / 39 脉案近景)~~ | Standard | **已并归上行** —— 世界内的纸在 P0 的读取形态 = 42 UI Toolkit 近景模态,**不是** UGUI world canvas mesh 文字 | ✅ **归档口径已裁(2026-09-21 · 第二十八批 · OQ-C3 结案)** | **裁由**:`OQ-48-4` 裁纸为「纯装饰不可拾 · 走近 → 42 近景模态」⇒ 该页的字号判据落在**纸面 UI 档**(UI Toolkit `--font-size-*` 主题变量,承 ADR-013 §三),而非「世界内 mesh 文字」档 —— 两档的判据原本不同(mesh 侧受屏幕占比与距离影响,UI Toolkit 侧是固定 px 基准),**归档即判据选择**,不是排版合并。**实测判据出处** = `design/ux/paper-closeup-48.md` §13(「正文 ≥24px @1080p」条),其「第七员登记」前置本批解除(登记落 `ADR-013 §十-B`)⇒ 自此可测。⚠️ **可测 ≠ 已测**:上行状态仍 `Not Started`,判据执行归实现轮(**禁借绿**)。⭑ 39 脉案近景 = **同族形态**(承 `interaction-patterns.md` P-02),一并走本档。
...
| 音量误置恢复(归零路径) | Standard | 44 总线 / 设置条目 | Not Started | **2026-09-25 登记(承 44 OQ-SS-7=甲)**:「逐格调音量 + 无归零 + sidecar 跨会话锁死」对运动/认知障碍玩家是**复合成本**(OQ-SS-5=甲 持久化后误置锁死跨会话)—— 载体 = **页级归源钮**(44 登记三件,验收 `AC-44-19`;控件形态归 42 元件轮);原 D-C 与本文件均未记账此成本,此行为补登 |
...
| Mono 音频 | **Standard 超额交付(2026-09-25 用户裁定 = 甲)** | 44 `mono_enabled` 设置暴露面 | Not Started | **原「不承诺 / 靠 OS 折中」与 `AC-44-13`[A] P0 在架 + 42 壳独听拨栓在架两件反向,已改判对齐**(升比降便宜:Unity 原生、ADR-018 已裁;Test Plan 见下) |
...
| 听觉四项(mono / 字幕默认开 / 七路独立音量 / 归源) | EditMode 断言 + 一次人工核 | `AC-44-13` · `AC-44-15` 半 · 七路枚举计数 · `AC-44-19` 重启保持 | 三门注册表断言全绿 + 归源重启复验 | qa-tester | Not Started(2026-09-25 二轮补:原 Test Plan 六行零听觉项,四项 Basic–Standard 承诺无排期) |
...
| **L-3** | **隔墙/隔门听觉检测**(44 主幻想通道)的视觉等价物 | Standard(不承诺) | 2026-09-18 裁定 D-C(承 44 `AC-44-12②`):幻想本体 = 「**看不到**还能听见」,视觉替代需视线、不能穿墙 ⇒ 该跨房间通道**无任何视觉等价可重建**;登记为显式代价 | 聋哑玩家在 P0 失去本设计的主张(仅剩脉案文字侧的间接信息) | ① 病人语声/口述**字幕**(Basic,上表);② 呼吸三类 cue 的**默认可见对应物**(`AC-13-F1`:姿态/面色/呼吸幅度 —— 非色相可辨子句承 44 规则九);③ 发行面(Steam 无障碍清单)如实披露 | 日期:**P1a 评审**;或任何「双通道重建跨房间信息」提案提出(即时复审) |
