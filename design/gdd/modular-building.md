...
> **参考**: `persistence-service.md`(7a,Approved 格式参照)· `world-and-ecozones.md`(6,缝一来源)· `game-concept.md:378-414`(医馆即机器)· `game-concept.md:695`(P0 范围口)
> **P0 范围口**: `game-concept.md:695` —— **固定医馆 + 小改造(允许更换家具/设施位置,不允许新建)**;P1a = 自由建造(719)
...
**P0 范围口 = `game-concept.md:695`:固定医馆 + 小改造(允许更换家具/设施位置,不允许新建)。**
玩家在 P0 不新建房间、不拆墙、不改槽位骨架 —— 只在一间固定医馆的烘焙槽位骨架内**重排**
家具与设施。这是「位置即逻辑」的最小验证形态:每一次挪动都改变某个数值(感染率 / 成药率 /
恢复),玩家**看得见**。P1a 才开放自由建造(`game-concept.md:719`),本 GDD 的架构为它留口
(模块目录 / 槽位约束是数据,不是代码)。
...
**P0 = 固定医馆 + 小改造**(`game-concept.md:695`):
...
**P1a = 自由建造**(`game-concept.md:719`):新建房间 / 拆墙 / 扩骨架 / **换模块**(评审裁定归 P1a,规则十)。
...
| **20** | 库存与物品 | `build_part` 库存裁决(判定 ⑤)· 返还结算 | 物品身份 = **20** | ⚠️ **半** —— 20 无 GDD;本 GDD 已列边,20 成文时须反向引用(同 6 的 `O-6-1` 模式) |
