...
| 45 联机(1-4  coop) | —— | 各端输入各自重映射 | 主机暂停语义须显式说明 | 精度取主机技能(ADR-018 §五)⇒ **听障玩家的远端读数不受影响,分叉的只有音** | Partial | 无障碍设置**不进流、不同步**(各端本地态)—— 此条须在 ADR #2(边界程序集)实现时守住 |
