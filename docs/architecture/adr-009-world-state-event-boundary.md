...
**已结(2026-09-23,不在本 ADR 裁决面上、由目标件结清)**:`OQ-10-9` —— `EmergencyAttempt` 的
**上行 QoS**,由 **ADR-001 §一之三 裁决一**(窄修订·同族三项)结清:判定输入类走**可靠通道上行**,
不走第二 QoS;「判定输入不得丢」与「保序非必需」的张力经上行二分消解(最新值 / 自愈类仍可走
第二 QoS)。本 ADR 原「不在本 ADR 打补丁」的边界**未被突破** —— 落点仍是 ADR-001。
