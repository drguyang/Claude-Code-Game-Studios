...
> **sim 实体数(未定 —— OQ-8,两处前置输入)**:
> `disease-simulation.md:1329-1335` 登记 **OQ-8**:`PatientState` 的**模拟范围**
> (在场才模拟 vs 全域模拟)未定 ⇒ 同场实体数无值。**该输入同时是两处的前置**:
> ① ADR-005 性能表 CPU 行的量纲(`实体数 × tick 频率 × 单次求值`);
> ② **ADR-017 §三 的 DOTS 复评门**(触发判据)。**须在写第一个 `Step` 之前标定。**

> **tick 频率(未定 —— OQ-25-8,与 OQ-8 同批前置 · 2026-09-17 由 25 GDD 回填 · R16)**:
> `combat-and-weapon-lines.md` 登记 **OQ-25-8**:tick 频率值未标定。25 的三处量纲全押在其上
> —— ① 占用门 `cooldown_ticks`(F-25-8);② 感知事件率上界(= tick 频率,承 ADR-016 §三 / ADR-020 §四);
> ③ `Decay_injury(Δtick)` 的半衰期参数(`trauma_half_life` 以 tick 计,归 OQ-25-7 数值轮)。
> **与 OQ-8 同性质:须在写第一个 `Step` 之前一并标定**(CPU 量纲 = 实体数 × **tick 频率** × 单次求值 —— 两个因子都无值)。
> **10 侧的同一缺口第三处表现(2026-09-18,承 10 首轮评审 · `OQ-10-10`)**:`emergency-procedures.md`
> F-10.6 把两个延迟预算分开 —— `L_input < 50 ms`(输入 → 呈现,10 自有)与
> `L_eval ≤ TICK_PERIOD`(9 的求值节奏)。⚠️ **`TICK_PERIOD` 在 10 侧是无量纲符号**(其值 = tick
> 频率之倒数,归本条),故 **`L_eval` 那条在 tick 频率标定前无法校验**;且「主机 `Append` 到体征
> 可见」的**实际**延迟 = `TICK_PERIOD` + 求值次序,不是 `L_input`。⇒ **与 `OQ-8` / `OQ-25-8` 同批裁定,不得各系统自填一个数。**

> **DOTS 复评门(ADR-017 §三,登记点)**:
> - **input**: OQ-8 标定完成
> - **trigger**: 同场实体数 ≥ **[阈值 — 待用户裁定]** 或 表现层帧时间占比 ≥ **[阈值 — 待用户裁定]**
...
- [ADR-004 ✅ Accepted 2026-09-15]**是否需要 DOTS** —— 由 **ADR-017** 兑现。
  **分层裁决:sim 侧永不上 DOTS(结构性排除 —— 门 A `"noEngineReferences": true` 与
  `Unity.Entities` 等引擎程序集天然冲突);表现层留复评门(触发 = OQ-8 标定 + 阈值待用户裁定)。
...
- [ADR-017 ✅ Accepted 2026-09-15]**是否引入 DOTS(sim 侧结构性排除 · 表现层留复评门)** ——
  ...
  **此结论不随规模变化**;② **表现层留复评门** —— 触发输入 = **OQ-8** 标定完成
  (`disease-simulation.md:1329-1335`,「在场才模拟 vs 全域模拟」),触发判据 = 同场实体数 / 表现层
  帧占比 ≥ **[阈值待用户裁定]**;作用面**仅表现层**(群集转向 / VFX / 渲染批处理 / Unity Physics 边界);
