...
- IL 扫描用 `System.Reflection.Metadata` 读编译产物 TypeRef/MemberRef(承 `AssemblyGates` b4「不引 Roslyn」口径)。
...
- **AC-44-B1**: 装配边界扫描。
  ...
  - Then: 三段全绿;负向夹具:在 44 命名空间临时加 `using DaYiJingCheng.Sim;` 引用 `IEventSink` ⇒ 测试必红。
  - Edge cases: 引擎模块(`UnityEngine.*`/Addressables/URP)不被误杀;`nameof`/反射字符串形态的引用同样被 IL 扫描抓住。
...
- **AC-44-B4**: 字段类型白名单。Given: 44 全类型。When: 反射扫字段类型。Then: ∈ {音频类型, BCL, `Sim.Contracts` DTO 族};负向:加一个 `DiseaseState` 类型字段 ⇒ 红。

## Test Evidence

**Story Type**: Integration
**Required evidence**:
- `tests/integration/audio_system/assembly_boundary_test.cs` — must exist and pass(或落 `tests/unit/` 扫描类,随门落点定)
