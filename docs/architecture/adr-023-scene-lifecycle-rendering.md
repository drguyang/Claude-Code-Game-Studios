...
>
> **2026-09-23 再增列(承 ADR-028 ②,同型同批)**:`AudioSource` / 音频发射器组件作为**场景内容**
> 出现在任何非 Boot 场景 = **构建失败**。理由同上:② 的一般禁令已存在(场景预摆 = 第二真源),
> 但原清单**未点名音频对象** ⇒ 预摆呼吸声发射器会被扫描放行,世界语境声源(ADR-028)与
> 动态锚点(F-44.7)脱节。**亦属已裁判据的扩列,非新裁决**;并进 S2 同批验。
...
- [ ] **S2** 「World.unity 零 gameplay 对象」构建期扫描判据可落地(场景 YAML 资产反序列化扫组件类型);
      ...
      **2026-09-23 扩充(ADR-028 ②)**:同扫描**再增列「`AudioSource` / 音频发射器在非 Boot 场景 = 构建失败」**,同批验
