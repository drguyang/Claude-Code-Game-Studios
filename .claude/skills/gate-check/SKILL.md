...
- [ ] Architecture traceability matrix has **zero Foundation layer gaps**
      ...
      > **`status: no-adr-by-design` 不计入 gap** —— 该态承载三类需求(**2026-09-23 QQ-08 扩第二类,用户裁定逐簇;同日 Epic-3 批扩第三类**):> ①「范围 / 政策声明类需求」
      ...
      > ③ **归属件落在另一系统的已 Approved GDD,且 ADR 段结构上不承载该决定** —— ADR 原文明写
      > defer / 只拥接口形状 / 归属件是数据内容;首簇 = 3 的 3 条(`TR-input-017/019/021`,
      > Epic-3 对症复核批 2026-09-23,归属件 = `emergency-procedures.md`)。
      > ⚠️ **② / ③ 类逐簇裁定,非自动豁免** —— 其它系统的 schema 簇 / 跨件归属簇仍须各自的用户裁定方可入 ◆)。
      > 由 TD 判定的既有要求不变且**三类共用**:一条需求若要转 `no-adr-by-design`,其**归属件必须已登记**,
      > 否则维持 `gap`。
