...
| TR | 缺什么 | 本轮判定 | 处置 |
|----|--------|---------|------|
| `TR-itemdb-031` | 掉落实体的世界状态事件化边界 | **ADR-009 §一/§二 已实质裁决**(派生态 + 身份进流/位置表现,`technical-preferences` ADR-009 条目可证)—— 缺的只是 registry 的 `adr:` 字段回写 | **登记同步,非新 ADR**。归回写轮;⚠️ 回写**不充当验收**(该条的 AC 落点仍在 21a 实现轮) |
