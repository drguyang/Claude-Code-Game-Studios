...
> 运行期零第三方行为树 / 寻路库**;**DOTS 已于 2026-09-15 由 ADR-017 裁定:
...
- [ADR-011 ✅ Accepted 2026-09-15]**输入架构(Input System 动作映射与急救延迟路径)** ——
...
- [ADR-016 ✅ Accepted 2026-09-15]**AI 架构(分层确定性 · 编辑器期行为程序烘焙 · 整数导航格寻路)** ——
  ...
  **运动表现态**(连续位移 / 动画 / 绕障)在表现层驱动,与 ADR-009 §五 的「掉落身份进流 / 位置表现」同构;
  ③ **行为载体 = 编辑器期行为树 → 构建期烘为整数数据**(走 ADR-014 管线;运行期零第三方行为树库);
  ...
  **§二 敌人实体复用 9 的伤情模型**:25 已把「生命归零」登记为 `INJ_COMA`,非致命模型(归零 = 昏迷)
  ...
  (病人 → 受伤实体),折叠谓词不适用于敌人行。
  **§三 感知输入 = 粗粒度整数格**:玩家跨格写世界流事件(频率 = 格穿越率,与帧率 / 位移距离无关),
  ...
  **§七 动物 AI 的 P1a 驯化接口本次定型**(`ITameable` + 复用行为程序的命令通道),
  ...
  Engine Knowledge Risk **MEDIUM** —— sim 侧判据纯 C#(**LOW**);抬到 MEDIUM 的是表现层两处
  (动态建造下的 NavMesh tile 更新成本 · 编辑器期行为树工具的导出 / 烘焙可行性)。
...
- [ADR-020 ✅ Accepted 2026-09-15]**玩家控制器与相机(CharacterController · 自建机位)** ——
  ...
  **跨格世界流事件**(连续位置 / 速度 / 朝向永不写流;**AC-20-03 BLOCKING**,grep 断言)。
  **这不是新裁决,是 ADR-016 §三 在玩家侧的对称落实** —— ADR-016 §三 定**读方**(13 / 27 读粗粒度整数格,
  `adr-016:181-190`),本 ADR §四 定**写方**(玩家跨格发事件);事件频率 = 格穿越率,**与帧率无关**
  ⇒ 不破坏 ADR-009 世界流有界性论证。
