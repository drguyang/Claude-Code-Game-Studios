...
| 层 | 内容 | 谁交付 | 17 侧状态 |
| --- | --- | --- | --- |
| **看得见的种类** | `quality_character[]` 给每档一个**定性修饰**(「新采带露」「干燥」…),由 42 以外观 / 药签措辞呈现 | **21a(数据)· 42(呈现)** | ✅ **R13 已裁 2026-09-25 = 甲(最小非空)** —— `quality_character[]` 自 D-21-16 改判为 **`MAX_QUALITY > 1` 时必填**(`item-database.md` §Schema B2),17 **自此可预期非空**(原「P0 可空 / 17 不得要求非空」口径废止);**过渡**:数据回写(P0 填词 + 门扩)未完成前该列可能仍空 ⇒ EXTERNAL 挂「待 21a 回写」而非「待裁」,**见注⑥** |
...
| **`quality_character[]` 为空** | ~~(P0 可空)~~ **✅ 2026-09-25 R13 = 甲:该情形在 `MAX_QUALITY > 1` 下被数据侧禁止**(D-21-16 改判必填,门扩后空列 = 构建期硬失败)⇒ **本行边界结构性消失**;**过渡期**(回写未完成):采集照常(品级仍在),42 仍无回退规格 —— EXTERNAL 注保留至回写落地 | `item-database.md` §Schema B2(R13=甲) |
...
> 非 17 的 EXTERNAL 断言);数据回写(P0 填词 + 门扩)完成前过渡挂「待 21a 回写」。
...
| **AC-17-05c** | [A] | **EXTERNAL · 待 21a 数据回写**(~~BLOCKED-BY-R13~~ —— **R13 ✅ 2026-09-25 已裁 = 甲**,裁定侧闭合;余 = 数据回写 + 本走查未跑) | `GIVEN` 21a 交付的 `quality_character[]`,`WHEN` 人工走查,`THEN` **取值在本系统语义成立**(无「陈放」「虫蛀」类**采后属性**)。⚠️ R13=甲 使该列必填 ⇒ **填词内容必须过 OQ-17-9 语义筛**(P0 无仓储/腐坏,采后属性不可达);数据回写前 17 侧不得记绿(注⑥)|
