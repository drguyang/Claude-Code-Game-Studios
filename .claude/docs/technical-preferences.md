...
- [ADR-006 ✅ Accepted 2026-09-14]**定点域边界数据契约** ——
  ...
  **2026-09-21 Amendment B 适用面第二次扩大(第二十六批 · 用户裁定)**:`player_id` 发号权威
...
- [ADR-010 ✅ Accepted 2026-09-15]**7a 持久化与存档格式(全二进制 codec · 单一义务清单)** ——
  ...
  系统 7a「持久化服务」是**全案唯一无 GDD 的 Core 系统**(⚠️ 2026-09-21 历史注:裁定时状态,
  后已立 `persistence-service.md`,2026-09-17),却被 ADR-005 / 006 / 007 / 008 / 009
  ...
  **残留**:`random-events.md` 仍写「存档头(或创建时的一条 `SimEvent`)」= 与 ADR-007 已 Accepted 裁决冲突,待修。
  ✅ **2026-09-21 复核:该残留已消除** —— 全仓 grep 该措辞**唯本行自引**,`random-events.md` 五处
  `WorldSeed` 相关表述(`:569` / `:581` / `:617` / `:1017` / `:1241`)均已收成「**存档头**(非 `SimEvent` ——
  ADR-007 §二 · ADR-010 §一)」口径,与 ADR-007 一致。本残留结案,原文不删,留作闭环记录。
...
- [ADR-013 ✅ Accepted 2026-09-15]**拟物 UI 框架(UI Toolkit 主 + UGUI 补 world/XR)** ——
  ...
  **⚠️ 2026-09-21 Amendment B(第二十八批「42 修订轮」· 用户裁定两项 = 方笺 + 戥子;增员是执行
  ...
  ⑦ Approved)**;`AC-42-F1` 走查整体仍 `NOT-RUN`(spec 成文 ≠ 走查执行;残余硬前置 = R8 / 45 联机夹具
  (~~`OQ-TUT-2`~~ / ~~`OQ-CP-2`~~ ✅ 均 2026-09-22 批裁结案),2026-09-22 `/ux-review` 轮回刷)。Engine Knowledge Risk **LOW**(本 Amendment 纯枚举与归属)。
...
- [ADR-019 ✅ Accepted 2026-09-15]**遥测与隐私(51 本地优先 · 回放即数据记录 · 零第三方上报)** ——
  ...
  却零 GDD / 零 ADR / 零 TR、估 S(⚠️ 2026-09-21 历史注:裁定时状态,51 后已立
  `telemetry-analytics.md`,2026-09-19 落盘)。一项核心用户裁定(照准):**51 本地优先**。
