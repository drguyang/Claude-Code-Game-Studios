...
| 层 | 内容 | 谁交付 | 17 侧状态 |
| --- | --- | --- | --- |
| **看得见的种类** | `quality_character[]` 给每档一个**定性修饰**(「新采带露」「干燥」…),由 42 以外观 / 药签措辞呈现 | **21a(数据)· 42(呈现)** | ✅ **R13 已裁 2026-09-25 = 甲(最小非空)** —— `quality_character[]` 自 D-21-16 改判为 **`MAX_QUALITY > 1` 时必填**(`item-database.md` §Schema B2),17 **自此可预期非空**(原「P0 可空 / 17 不得要求非空」口径废止);**✅ 数据回写完成(2026-09-25)**(P0 填词 5 档 + 门扩 + fixture,烘焙 items 604B)⇒ EXTERNAL「待回写」解除,17 可按非空消费,**见注⑥** |
...
| **`quality_character[]` 为空** | ~~(P0 可空)~~ **✅ 2026-09-25 R13 = 甲:该情形在 `MAX_QUALITY > 1` 下被数据侧禁止**(D-21-16 改判必填,门扩后空列 = 构建期硬失败)⇒ **本行边界结构性消失**;**✅ 回写完成(2026-09-25)**:空列已是构建期硬失败(数据不可能为空)⇒ 本行**关闭**(无回退路径需要登记) | `item-database.md` §Schema B2(R13=甲) |
...
> ① ~~不得要求非空~~ **✅ 2026-09-25 R13 = 甲 改判(2026-09-19「暂不裁」已被取代)** ——
> 用户后裁 **「`MAX_QUALITY > 1` 时最小非空」**(含成药侧),D-21-16 已改判落笔 ⇒
> **非空从此是 21a schema 侧义务**,17 仍**不自己提要求**(归属不变:要求来自 21a 必填门,
> 非 17 的 EXTERNAL 断言);**✅ 数据回写完成(2026-09-25)—— 「待 21a 回写」过渡结束**,17 按非空消费。
> ② **但可登记一条独立的新发现给 21a**(⚠️ **R13=甲 后本条权重上升 —— 填词时必须遵守**):
> `quality_character[]` 的**取值语义**在本系统不成立 ——
> 「**陈放**」「**虫蛀**」是**采后 / 储藏期**属性,而 **P0 无仓储系统、无腐坏机制** ⇒
> 这两档修饰**在 P0 不可达**(医学专家受众的**首次出戏点**,硬约束)。
> ⇒ 登记 **`OQ-17-9`**,4 项 / 42 / 21a 同批;**21a 数据回写填词时须先过 OQ-17-9 语义筛**
> (**✅ 已过筛落盘 2026-09-25**,见 OQ 表结案行)。
...
> ✅ **2026-09-21 闭合本条**:前提「6 尚未撰写」陈旧(`world-and-ecozones.md` 已成稿 Approved),
> 且**要求的边已落** —— 6 的下游表已列「17 采集」(`world-and-ecozones.md:425` / `:809`,
> 2026-09-20 回刷为 ✅ 双向)。**本块与本件 `:444` 原为同件自相矛盾**(一处说已双向、一处说未撰写),
> 现收敛为「已闭合」;原登记文字不删,留作闭合记录。
...
| **AC-17-05c** | [A] | **EXTERNAL · 数据已回写,走查未跑**(~~待回写~~ —— **✅ 回写完成 2026-09-25**:5 档词经 OQ-17-9 筛 + 用户审定落盘;`BLOCKED-BY-R13` 同日已裁 = 甲) | `GIVEN` 21a 交付的 `quality_character[]`,`WHEN` 人工走查,`THEN` **取值在本系统语义成立**(无「陈放」「虫蛀」类**采后属性**)。✅ P0 词集(`枯脆细碎/皮薄色暗/条匀皮厚/条肥色正/皮厚丝丰`)**零采后词**,语义筛已过;**本 AC 仍 `[A]` 未跑** —— 走查执行时核词面,禁借回写记绿(注⑥)|
...
| **OQ-17-9** | `quality_character[]` 的**取值语义**(「陈放」「虫蛀」在 P0 无仓储 / 无腐坏 ⇒ 不可达)| 用户 + 21a / 42 | **✅ 结案 2026-09-25**(与 R13 同批兑现,不捆绑)—— **P0 实词已落盘且全数过筛**:原料 5 档(`枯脆细碎/皮薄色暗/条匀皮厚/条肥色正/皮厚丝丰`,采时可观察)+ 成药 5 档(`浑浊沉淀/色浊欠匀/清亮尚匀/澄明匀净/澄澈晶莹`,工艺过程可观察),**零采后储藏词**,词集经用户审定;上游 GDD §B2 废例已加「勿抄」注 | 医学专家受众首次出戏点(硬约束)—— 筛已执行,走查仍归 17/42 |
