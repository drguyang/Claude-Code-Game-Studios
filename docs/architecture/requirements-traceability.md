...
> 人读矩阵:`docs/architecture/traceability-index.md`(GDD → ADR 两级;**314 ✅ / 79 ⚠️ / 104 ❌ / ◆2 no-adr-by-design** —— 2026-09-21 D-R3 专门批次后实测;上轮值 245/51/89 系分母不含 12 项零 TR 系统的偏高口径)
>
> **2026-09-21 更新(D-R3 专门批次 —— 12 项 P0 零 TR 系统回填,+112 条;387 → 499)**
> —— **314/79/104/◆2**,`yaml.safe_load` 逐条复算自洽 499(既有条目逐字未动,append-only);
> ⚠️ 覆盖率 63.0% → **62.9%** 是分母效应(账外收进账内),**非质量下降**。
...
| ADR | Architectural decision governing implementation | ⚠️ 314 covered / 79 partial / **104 gap**(另 2 条 ◆ `no-adr-by-design`,2026-09-21 起为第四态,不计缺口)|
...
| NO STORY — ADR exists, not yet implemented | 314 | 62.9% | registry `status: covered` 计数 |
| NO ADR — architectural gap | 104 | 20.8% | registry `status: gap` 计数(D-R3 回填 +15;2026-09-21 曾由 91 减 ◆2)|
| NO ADR BY DESIGN — 范围 / 政策声明,结构上无裁决可挂 | 2 | 0.5% | registry `status: no-adr-by-design` 计数 —— **不计入缺口**;判据见 `traceability-index.md` 读法表 ◆ 行 |
| PARTIAL(链已断在 ADR 段)| 79 | 15.8% | registry `status: partial` 计数(D-R3 回填 +28)|
| **Total requirements** | **499** | **100%** | `tr-registry.yaml` |

> 计数口径:对 registry 的 `status:` 字段直接 `yaml.safe_load` 计数,与
> `traceability-index.md` §汇总「合计」行一致(**314 + 79 + 104 + ◆2 = 499**;2026-09-21 起四态,D-R3 批次后值)。
...
104 − 2 = **102 条** gap 分布在非 Foundation 层(2026-09-21 **D-R3 批次后值**:87 + 本批新增 15 ——
